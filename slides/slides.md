...
<div class="mt-5 pt-4 border-t border-slate-300 dark:border-slate-700 flex items-center text-lg opacity-70">
<div class="font-mono text-lg w-14">2부</div>
<div class="font-bold w-44">도구·데이터 연결</div>
<div>커넥터·MCP로 내 자료 붙이기 <span class="opacity-70">(별도 세션)</span></div>
</div>
...
2부 구간 추가(이슈 #52 Task 4): 01~07과 구분선으로 갈라 번호를 잇지 않고 「별도 세션」으로 표기. 2부는 오늘 90분에 포함되지 않는다. 이 표기가 유일한 자리다(2부의 흐름 슬라이드에 있던 같은 안내는 이슈 #52 Task 6에서 제거). 덱에서의 물리 위치는 How 뒤·실습 앞이다.
-->
...
> 사이트·슬라이드에서 만나게 될 **두 용어**와, **어디까지 다루는지**를 한 번에 짚습니다.
...
→ **연결하는 방법은 2부에서 다룹니다.**<br/>
단, RAG 내부 구조는 2부에서도 **범위 밖**.
...
청중 메시지: 「이 단어들이 본 강의에 나오면 멈추지 마세요. 실제로 연결하는 방법은 2부에서 화면 기준으로 다룹니다. 다만 마크다운 문법 학습과 RAG 내부 구조는 그때도 밖입니다.」
접합부 전환(이슈 #52 Task 4): 종전에는 설치와 내부 구조를 한데 묶어 범위 밖으로 선언했으나, docs/basics.md 「외부 컨텍스트」 절의 현행 서술과 같은 구도로 바꿈. 연결 방법은 2부로 넘기고, RAG 내부 구조가 범위 밖이라는 선언만 남긴다.
...
<div class="mt-6 text-center text-sm opacity-75 italic max-w-3xl mx-auto">
"프롬프트로만 진행"의 짝 원칙: 손으로 안 쓰지만 검토는 합니다. 사례: <code>step04</code>의 인식 엑셀 검수 단계.
</div>
...
이슈 #52 Task 6 보정: 하단 참고 문단을 네 줄로 나눔. 한 줄 나열이라 「Claude.ai 한도」 링크가 줄 끝에서 쪼개지고 수치 셋이 구분 없이 붙어 읽혔음. 라벨 / claude.ai 기준 수치 / Claude Code 기준 수치 / 외부 링크로 갈랐고, 원문의 쌍반점 구분을 줄바꿈이 대신함. 줄 간격은 leading-relaxed.
-->
...
이슈 #52 Task 6 보정: 하단 보조 설명(별도 세션·이용까지 경계) 제거. 「별도 세션」은 「오늘의 흐름」 2부 행이 라벨로 이미 달고 있고, 범위 경계는 7장 「시작하기 전에」가 다루지 않는 것까지 구체로 편다. 목차 장에서 먼저 말할 필요가 없다.
-->
...
<div class="mt-4 border-l-4 border-teal-400 pl-5 py-2 bg-teal-50 dark:bg-teal-900/15 rounded-r max-w-4xl mx-auto text-sm">

선택이니 <strong>연결하지 않는 쪽이 맞는 경우도 있습니다.</strong> 누구나 볼 수 있게 공개된 자료는 주소를 알려 주면 되고, 한 번 쓰고 마는 자료는 붙여넣는 편이 빠릅니다.<br/>
연결이 필요해지는 것은 <strong>로그인과 권한 확인이 필요한 일</strong>입니다. 로그인해야 볼 수 있는 자료를 읽는 것도, 공개된 자료를 고치거나 남기는 것도 여기 해당합니다.
...
이슈 #52 Task 6 추가: 「연결하지 않는 쪽이 맞는 경우」 한 줄. 공개 저장소를 읽는 데 커넥터가 필요 없다는 지적(63장)이 GitHub만의 이야기가 아니라 MCP 서버에도 걸려서, 「선택」을 말하는 이 장에 기준을 한 번 세워 둔다. 웹 공개 자료와 1회성 자료를 제외로 두고, 필요 조건은 인증·인가 두 갈래로 못박는다: 읽기부터 로그인이 필요한 자료, 그리고 읽기는 공개여도 고치고 남기는 일에 계정이 필요한 경우. 초안에서 「반복해서 쓸 때」로 썼다가 사용자 지적으로 고쳤다. 반복은 연결이 편해지는 이유이지 연결이 있어야만 되는 이유가 아니다. 두 갈래는 결국 한 기준이라 「로그인과 권한 확인이 필요한 일」로 묶고 읽기·쓰기를 예로 붙인다. 「내 계정이라야 되는 일」로 썼다가 사용자 지적으로 고쳤다: 공용 계정으로 하는 일도 있어 소유자를 특정하면 좁아진다. 비개발자 청중이라 인증·인가라는 말 대신 「로그인과 권한 확인」으로 푼다. docs/connect/index.md #from-part1이 SSoT.
-->
...
- **회사 환경이라면 회사 정책을 먼저 확인하고, 회사가 허용한 것만 씁니다.** 커넥터와 MCP 서버 모두 같습니다.
...
#59: 부서 지목·문의 유도 문구 제거, 회사 정책으로 통일. 준비물 둘째 항목을 docs 준비물의 한 줄(정책 확인 후 허용된 것만, 커넥터·MCP 서버 공통)로 맞추고, 목록에 안 보이면 권한 탓일 수 있다는 둘째 문장을 뺐다. 요청하면 열린다는 뜻으로 읽혀서다.
-->
...
내 계정이 회사 자료 대부분에 접근할 수 있다면 그만큼 넓어집니다.
...
<div class="mt-8 max-w-4xl mx-auto text-center text-lg">

**"나는 권한이 넓은 편인가"** 를 한 번 떠올려 보세요. 넓다면 줄일 방법이 있습니다.
...
승인 화면에 선택지가 없더라도 두 가지로 줄일 수 있습니다.

<div class="mt-8 grid grid-cols-2 gap-5 max-w-5xl mx-auto">

<div class="border-l-4 border-blue-400 pl-5 py-4 bg-blue-50 dark:bg-blue-900/15 rounded-r">
...
<!--
docs/connect/connectors.md `#approval-screen` 말미 두 항목 압축: ADR-0002 단방향 파생.
이슈 #52 Task 6에서 앞 장(「내 권한 그대로」)에서 갈라 나온 장. 승인 화면에 선택지가 없을 때의 대안을 주는 것이 요점이라 앞 장 바로 뒤에 붙여 둔다. 「줄일 방법이 없다」로 끝나면 청중이 승인 화면에서 할 일이 없어진다.
두 방법 모두 승인 화면 밖의 시점이다: 하나는 대화에서 켤 때, 하나는 AI가 물어올 때.
-->
...
커넥터가 필요해지는 것은 **로그인해야 보이는 저장소**이거나 **이슈에 코멘트를 남기는 일**입니다.
...
커넥터는 **로그인과 권한 확인이 필요한 일**에 쓰는 수단입니다. 회사 자료를 다루는 연결의 공통 기준(플랜·회사 정책)은 **다른 커넥터와 같습니다.**
...
#59: 하단 띠의 정책 명칭(관리자·조직·사내로 흩어져 있던 표현)을 「회사 정책」으로 통일. docs·slides의 정책 명칭을 하나로 맞춘 변경이다.
-->
...
**회사 정책을 먼저 확인하고, 회사가 허용한 커넥터만 씁니다.** Team·Enterprise 플랜에서는 **조직 관리자가 먼저 커넥터를 켜 주어야** 구성원이 쓸 수 있습니다.

회사는 보안 정책에 따라 커넥터를 막아 두었을 수 있습니다. 허용되지 않은 커넥터는 쓰지 않고, 연결도 회사가 정한 방법으로 합니다. 목록에 없으면 **허용 범위 밖**입니다.
...
#59: 부서 지목·문의 유도 문구 제거, 회사 정책으로 통일. 오른쪽 카드를 docs #enterprise 첫 문단 요약(정책 확인 → 허용된 것만 → 막아 두었을 수 있다 → 허용되지 않은 것은 쓰지 않음 → 연결은 회사가 정한 방법 → 목록에 없으면 허용 범위 밖)으로 바꿨다. 「내 탓이 아니라 조직 탓」 식의 문장은 요청하면 열린다는 뜻으로 읽혀 뺐다.
-->
...
화면에서는 <strong>둘 다 「커넥터」</strong>입니다. 목록에는 Anthropic이 검토한 항목(이름 옆 체크 표시)과 선별만 거친 항목(「Community」 라벨)이 섞여 있으니, <strong>「Community」 라벨이면 만든 곳을 한 번 더</strong> 보고 연결하세요.
</div>
...
**접근 범위**: **인터넷에서 열려 있는 주소**까지입니다. 내 컴퓨터 안의 파일이나 사내망 주소에는 접근하지 못합니다.
...
**B가 회사 사내망 MCP 서버를 쓰는 방식입니다.** 접속이 내 컴퓨터에서 나가니, VPN이 붙어 있으면 사내 주소에 접근할 수 있습니다.
...
**사내망 주소는 내가 VPN에 붙어 있어도 접근하지 못합니다.** 접속하는 쪽이 내 컴퓨터가 아니기 때문입니다.
...
회색 점선 간선(접속할 수 없음)이 이 장의 메시지다. 사내망에 접근하지 못한다는 것을 말이 아니라 그림에서 먼저 본다.
...
회사가 사내 자료용 서버를 제공하면 **방법은 회사가 정합니다.** 주소·계정·설치 파일을 임의로 구해 쓰지 말고 **회사가 정한 방법을 그대로 따르세요.**
...
#59: 파란 박스의 절차 준수 문장 주체를 부서에서 회사(정책)로 바꿨다. docs #remote-vs-local과 정합.
-->
...
| 확인할 것 | 무엇을 보나요 |
|---|---|
| **목록의 검토 표시** | 목록에서 고르는 경우, 이름 옆 체크 표시(Verified)인지 「Community」 라벨인지 |
...
자세한 기준은 사이트 페이지 <code>보안 및 개인정보 가이드</code>의 「외부 연결」 절에 있습니다.
</div>
...
<!--
docs/security-guide.md `## 정리` 표 파생: ADR-0002 단방향 파생.
#59: 임직원 행 「연결은 회사가 허용한 것만」을 docs와 같은 자리(회사 계약 플랜 행 아래)에 추가했다.
-->
