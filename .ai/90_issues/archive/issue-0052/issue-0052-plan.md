...
- **완료 기준**:
  ...
  - [D] 구성안이 정한 1~17장이 그 제목·순서로 존재한다 (전수 판정은 Task 3 게이트가 맡으며, 이 게이트는 앞 17장만 본다). Task 6에서 13장을 두 장으로 나눠 이 구간이 16장에서 17장이 됐다
    <details>
    <summary>검증 명령 ― repo 루트에서 실행, 출력 0건이면 통과</summary>
    ...
    # 점검하고 끊기 · 회사 환경이라면
    EOF
    ) <(awk '/^# 2부 ― 도구·데이터 연결$/{f=1} /^# 실습$/{f=0} f && /^# /' slides/slides.md | head -17) \
      || echo '위반: 2부 앞 17장의 제목·순서 불일치'
    ```

    - 설계 주의: 위 heredoc은 들여쓰기 없이 복사해 실행한다(들여쓴 `EOF`·본문은 `<<-`가 아니면 그대로 비교값에 들어간다). `head -17`로 앞부분만 보므로 Task 3이 18~32장을 뒤에 붙여도 이 게이트는 계속 유효하고, 앞 17장 사이에 끼워 넣으면 불일치로 잡힌다. 구간에 제목이 17개보다 적으면 diff가 누락 행을 출력한다.
    </details>
...
- **완료 기준**:
  - [D] 2부 32장 전부가 그 제목·순서로 존재하고 그 외 장이 0건이다 (구성안 31장 + Task 6에서 13장을 나눠 늘어난 1장)
    <details>
    <summary>검증 명령 ― repo 루트에서 실행, 출력 0건이면 통과</summary>
    ...
    EOF
      ...
      || echo '위반: 2부 32장의 제목·순서·전수 불일치'
    ```

    - 설계 주의: 위 heredoc은 들여쓰기 없이 복사해 실행한다. 구간 전체를 비교하므로 누락·순서 뒤바뀜과 함께 **기대 목록에 없는 장 0건**까지 한 번에 판정한다. Task 2의 게이트는 앞 17장만 보므로 전수 판정은 이 게이트에만 있다.
    </details>
  - [D] Slidev 빌드 통과  (검증: spec 완료의 정의 첫 항목의 명령 재실행)
