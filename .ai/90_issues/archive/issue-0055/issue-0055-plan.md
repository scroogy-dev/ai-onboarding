...
- **완료 기준**:
  - [D] 홈 배정표가 이 문서에 표로 존재하고 행이 15개 이상이다
    ...
    ```bash
    P=.ai/90_issues/archive/issue-0055/issue-0055-plan.md
  ...
  - [D] summary Task 1의 `수행 내용 요약`에 DoD 게이트 8종의 착수값이 기록된다 (`게이트 착수값:` 행 존재)
    ...
    ```bash
    S=.ai/90_issues/archive/issue-0055/issue-0055-summary.md
...
- **작업 내용**:
  ...
  3. **인용부호 표기를 문장 부호 규정에 맞춰 통일한다.** 「한글 맞춤법」 부록 *문장 부호* 기준으로, **소제목·항목명·메뉴명 등 명칭을 가리키는 자리는 홑낫표**(`「 」`), **마음속으로 한 말과 특별히 드러내는 부분은 작은따옴표**(`' '`), **직접 대화·직접 인용은 큰따옴표**(`" "`)다. 프롬프트 예시는 직접 인용이라 큰따옴표가 맞으므로 낫표로 바꾸지 않는다. 큰따옴표가 남은 행을 전수로 뽑아 자리별로 판정하고, 판정 결과를 summary에 남긴다.
     - `docs/basics.md`의 소제목 `### 하네스는 결국 "내가 세팅하는 것들"`은 그 제목을 인용하는 다른 페이지(`connect/connectors.md` 13행 링크 텍스트 등)가 함께 따라가야 하므로, 이 Task에서 **제목과 인용처를 한 번에** 고친다. 페이지별 Task에서 한쪽만 고치면 표기가 갈린다.
  4. 사용자가 nav 순서대로 전체를 통독하고 페이지 간 흐름 피드백을 준다. 피드백이 소진될 때까지 반영한다.
  5. 착수 커밋(`6bad27c`) 대비 최종 diff의 규모(줄 수·카드 수 전후)를 summary에 기록한다. #56이 대조 범위를 잡는 데 쓴다.
- **완료 기준**:
  - [D] spec DoD `[D]` 항목 8종 전부 통과  (검증: 해당 명령 재실행)
  - [ND] 큰따옴표가 남은 자리가 전수로 판정되고, 명칭을 가리키는 자리에 큰따옴표가 0건이다  (검증: 아래 명령으로 목록을 뽑아 사람이 자리별로 판정)  ← 강등 사유: 직접 인용과 명칭 지칭은 문맥 판단이라 기계로 가를 수 없다
    <details>
    <summary>검증 명령 ― 출력된 행을 자리별로 판정</summary>

    ```bash
    find docs labs/README.md -name '*.md' | sort | while read -r f; do
      awk -v F="$f" '/^[[:space:]]*```/{c=!c; next} !c && /"[^"]+"/ {print F":"NR":"$0}' "$f"
    done | grep -vE '<[a-z]+ [a-z-]+=' | grep -vE '^[^:]+:[0-9]+:[[:space:]]*(!!!|\?\?\?)' | grep -vE ':[[:space:]]*-[[:space:]]*"'
    ```

    - 설계 주의: 마크업 문법이라 판정 대상이 아닌 것 4종을 전부 빼고 남은 행이 판정 대상이다. ① 코드 블록 안(awk의 펜스 토글) ― mermaid 노드 라벨 `A["…"]`이 여기서 대량으로 걸린다. admonition 안에 들여쓴 펜스도 잡도록 `^[[:space:]]*` 를 둔다 ② HTML 속성(`class="grid"` 등) ③ admonition 유형 뒤의 라벨 ④ `- "…"` 불릿 ― 프롬프트 예시(직접 인용)라 규정상 큰따옴표가 맞다.
    - 착수값: 45건 (`index.md` 14 · `basics.md` 8 · `security-guide.md` 8 · `operation-guide.md` 5 · `mcp-servers.md` 3 · `labs.md` 2 · `labs/README.md` 2 · `connectors.md` 1 · `connect/index.md` 1 · `intro.md` 1). `operation-guide.md` 5건은 Task 7의 파일 삭제로 함께 사라진다.
    </details>
