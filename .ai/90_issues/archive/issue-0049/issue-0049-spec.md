...
- **연결 경로 축 정정(2026-08-05 추가)** — 「설정 위치」와 「MCP 서버 위치」를 독립된 두 축으로 분리하고, 누락된 **로컬 설정 + 원격 MCP** 조합(사내망 MCP 서버)을 임직원 확장으로 반영. 대상: `docs/connect/mcp-servers.md`(주), `docs/connect/index.md`·`docs/intro.md`·`docs/security-guide.md`·`.ai/40_domain/glossary.md`·ADR-0010(파급)
- **GitHub 접근의 트랙별 입장 반영(2026-08-08 추가)** — GitHub을 소스코드 관리 도구로 규정하고, 학생·일반인(개발에 관심이 있으면 접근 필요)과 임직원(별도 업무 도구 우선, 회사 정책 확인이 먼저, 사내 GHE 로그인·라이선스 제약으로 지양)을 구분해 서술. 대상: `docs/connect/connectors.md`(주), `docs/connect/index.md`·`docs/security-guide.md`·`mkdocs.yml`(파급)
...
- [ ] [D] 사내망 MCP 서버 케이스(로컬 설정 + 원격 MCP)가 `mcp-servers.md`에 서술됨  (검증: `grep -c '사내망' docs/connect/mcp-servers.md` ≥ 1)
- [ ] [QD] 「설정 위치」와 「MCP 서버 위치」가 등치되지 않고 독립된 두 축으로 서술되며, 접속 주체가 경로별로 구분됨  (검증: 다른 AI가 채점, 별도 세션)  ← 강등 사유: 두 축의 분리 여부는 서술 의미 판단이라 명령으로 환원 불가. 이 항목이 최초 작성 시 무너진 지점이므로 audit 대조 대상으로 명시한다
- [ ] [D] `connectors.md`에서 Atlassian 절이 GitHub 절보다 앞에 위치하고 사내 GHE 제약이 서술됨
    <details>
    ...
    앵커가 하나라도 없으면 행 번호 비교가 공집합이 되어 그대로 통과하므로, `!a||!g`로 두 앵커의 실재를 함께 판정한다. 순서 판정을 셸 변수·`[ ]` 비교로 짜면 zsh에서 값이 비어 위반이 묵음 처리되므로 awk 한 줄로 고정한다.
    </details>
- [ ] [QD] GitHub 접근이 트랙별로 구분 서술되고, 임직원 지양의 근거(별도 업무 도구 우선·GHE 로그인·라이선스 제약·회사 정책 우선)가 담김  (검증: 다른 AI가 채점, 별도 세션)  ← 강등 사유: 트랙별 입장의 전달 여부는 서술 의미 판단이라 명령으로 환원 불가
...
- **도구 기준(2026-08-05 정정, 사용자 확인 완료)** — 최초 작성 시 작성자가 단독으로 "비개발자이므로 UI 경로만"이라고 좁혀 두었으나, 이 전제는 Task 0 승인 질의에 오르지 않은 항목이었다. 사용자 확인 결과 아래로 정정한다.
  - **연결 경로는 두 축으로 나눠 다룬다.** ① **설정을 어디에 하나** — 계정에 등록하는 UI 커넥터 / 내 컴퓨터에 설정하는 로컬 커넥터·확장 프로그램 ② **MCP 서버가 어디서 도나** — 로컬 실행 / 원격 주소. 두 축은 독립이며, **로컬 설정 + 원격 MCP** 조합(회사 사내망 MCP 서버를 내 컴퓨터 설정으로 연결)이 실제 임직원 케이스다.
  - **접속 주체가 경로마다 다르다.** UI 커넥터는 Anthropic 클라우드 인프라가 접속하고(사내망 주소는 방화벽에 Anthropic IP 허용 필요), 로컬 커넥터·확장 프로그램은 내 컴퓨터가 접속한다(VPN이 붙어 있으면 사내망 주소에 닿음). 근거: [원격 MCP 사용자 정의 커넥터](https://support.claude.com/ko/articles/11175166) 「네트워크 요구사항」, [Cowork 사용하기](https://support.claude.com/ko/articles/15520349) 「데스크톱 앱이 필요한 기능」.
  - **트랙 구분**: 공개된 커넥터·원격 MCP 서버는 **일반론**(학생·일반인 포함 전원), 사내망 MCP 서버 연결은 **임직원 확장**으로 배치한다.
  - **설정 실물은 보여주지 않는다.** 임직원 확장도 "회사가 안내한 대로 등록" 수준까지만 쓰고, `~/.claude.json` 등 설정 파일 실물은 다루지 않는다 — 실물 가이드는 후속 실습 자료(`labs/`)가 담당한다.
  - **로컬 설치 형태**: 확장 프로그램 설치와 MCPB 형태 설치가 있다는 정도만 언급하고 상세는 다루지 않는다.
- **GitHub 접근에 대한 입장(2026-08-08 추가, 사용자 확정)** — 최초 작성 시 기본 커넥터 두 서비스를 트랙 구분 없이 대등하게 권했으나, GitHub은 트랙에 따라 권장 방향이 갈린다. 아래로 정한다.
  - **도구의 성격을 먼저 밝힌다.** GitHub은 기본적으로 **소스코드를 보관하고 변경 이력을 관리하는 도구**다. 문서·이슈가 쌓이는 곳으로 규정하지 않는다.
  - **학생·일반인**: 개발에 관심이 있으면 접근이 필요하다. 공개 저장소는 계정 없이도 읽을 수 있어 문턱이 낮다.
  - **임직원**: 이슈·업무 관리를 위한 별도 도구(Confluence·Jira 등)가 있으면 GitHub 연결을 **지양**한다. 개발자와 비개발자가 함께 봐야 하는 내용이라면 그 별도 도구에서 관리하는 편이 낫다. 사내 GHE(GitHub Enterprise)는 **공개 저장소라도 로그인이 필요하고 좌석 라이선스가 걸려 있어**, 비개발자 접근 자체가 정책·비용 판단의 대상이 된다. **회사 정책 확인이 먼저**다.
  - **지양의 대상은 회사 자료를 다루는 연결이지 GitHub 자체가 아니다.** 이 교육의 실습 자료도 공개 저장소로 배포하므로(`docs/labs.md`), 학생·일반인이 위축되지 않도록 선을 문장에 남긴다.
  - **배치**: 정책 본문은 `docs/security-guide.md` 임직원 절(SSoT), 도구 성격과 트랙별 판단은 `docs/connect/connectors.md` GitHub 절.
  - **절 순서**: 온보딩 기본값을 순서로도 전달하기 위해 **Atlassian을 GitHub보다 앞**에 둔다. 앵커(`#github`·`#atlassian`)는 다른 문서가 링크하므로 유지한다.
