...
이때 '재구성'은 모델+하네스 섹션 배치에 그치지 않고, **이론 파트 3개 문서(`index`·`intro`·`basics`) 전체의 중복을 걷어내고 한 흐름으로 잇는 전면 재배치**를 포함한다. (모델+하네스는 그 재배치의 한 축으로 편입)
...
- **이론 파트 3개 문서(`docs/index.md`·`docs/intro.md`·`docs/basics.md`) 전체 재배치** — 핵심 대비(챗봇/에이전트·매번 시키기/재사용·1회성/스킬)의 **중복 제거**, 섹션별 역할(축) 분담, 읽기 흐름 재설계
- 위 재배치를 SSoT `docs/` 본문에 **직접 반영** (제안에 그치지 않음). 단계적 반영 + 사용자 점검 루프로 진행
- 필요 시 `glossary.md` 최소 보강
- **이론 본문(`docs/`) 확정 후 슬라이드(`slides/`) 파생 반영** — 발표용 압축본이라 1:1 미러링은 아니며, 누적 미반영분(바이브/에이전틱 잔존·Antigravity·Opus 버전 등)까지 정합 (2026-06-01 범위 편입)
...
- [ ] 핵심 대비의 4중 중복이 정리되어 **각 섹션이 고유 축**을 갖고 교차링크로 연결되었다
- [ ] 사용량 확인 등 **문서 간 중복이 단일 집**으로 정리되었다
- [ ] 이론 파트가 **중복 없이 한 흐름**으로 읽힌다 (전체 재배치가 `docs/`에 반영 완료)
- [ ] 확정 결과가 SSoT인 `docs/` 본문(및 필요 시 `glossary.md`)에 반영되었다
- [ ] 이론 본문 확정 내용이 발표용 슬라이드(`slides/`)에 반영되었다 (docs/slides divergence 해소)
