...
>
> **공통 검증 필터**: 아래 Task들의 잔존 줄표 검증은 spec 완료의 정의 1번 `[D]`의 마크다운 필터(링크 라벨·표 기호 제거 + 제목·admonition 제목·범례 행 제외)를 대상 파일만 바꿔 재사용한다. HTML 자산은 spec 완료의 정의 2번 `[D]`의 별도 필터를 쓴다. 필터에 예외를 추가하면 spec 접기 주석과 이 파일 양쪽에 남긴다.
...
- **작업 내용**:
  1. `labs/` 하위 `*.md` 전부의 본문 산문 줄표를 기준대로 전환한다.
