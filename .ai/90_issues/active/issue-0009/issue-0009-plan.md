...
- **작업 내용**:
  1. https://scroogy-dev.github.io/ai-onboarding/ 접속 — 기존 문서 사이트 정상 동작 확인 *(머지·배포 후)*
  2. 슬라이드 경로(예: `/slides/`) 접속 — 슬라이드 정상 렌더링·내비게이션 동작 확인 *(머지·배포 후)*
  3. README 또는 `docs/`에 Slidev 로컬 실행 명령, 슬라이드 URL 안내 추가 *(현재 브랜치)*
  4. ADR-0002의 단방향 동기화 원칙에 맞춘 **`docs/` → `slides/` sync 체크리스트** 작성·게시 (발표 마일스톤 직전 점검 항목) *(현재 브랜치)*
  5. Slidev 슬라이드의 콘텐츠·내비게이션·디자인이 자체 구현 슬라이드를 충분히 대체하는지 최종 점검 — 통과해야 Task 6 진행 *(머지·배포 후)*
