...
- 동승 후보 2종 흡수 본문 작성 (Task 1, 2026-05-09 결정)
  - 4. 컨텍스트 유지·새 대화 기준 ("대화방 위생") — 결이 핵심 3종과 같아 같은 도입 박스 형태로
...
- [x] 동승 후보(4 컨텍스트 위생·2 페르소나) 흡수/분리 결정 — 둘 다 본 이슈 흡수 (2026-05-09)
- [ ] docs 도입 섹션에 핵심 3종 + 동승 2종(컨텍스트 위생·페르소나) 총 5종이 admonition/카드 형태로 반영됨 (의도적 강조 유지)
- [ ] slides 도입부에 같은 5종이 동기화됨 (한국어 sans-serif 강제·발표 톤 유지)
- [ ] `labs/README.md`·`docs/labs.md`에 "결과물 검토" 원칙이 시리즈 차원으로 호출됨
- [ ] ADR 정책화 여부가 결정되고, 신설/보강이 필요하면 해당 ADR 파일이 작성·갱신됨
- [ ] 후속 후보 노트(2026-05-08)의 항목 1·2·3·4·8 상태가 "이슈 #32 반영 완료"로 갱신됨
- [ ] 메모리 후보 `feedback_labs-review-output.md` 신설 여부 결정·반영
- [ ] MkDocs 빌드·Slidev 빌드(해당 시) 정상
- [ ] PR 생성·리뷰·머지 완료, 본 이슈가 archive로 이관됨
