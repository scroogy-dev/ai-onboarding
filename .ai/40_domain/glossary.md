...
| MCP 서버 | 기본 커넥터 목록에 없는 도구·자료를 **직접 찾아 등록해** 연결하는 방식. 원격(화면 메뉴 이름은 「사용자 정의 커넥터」, 서버 주소 등록)과 로컬(Claude Desktop 확장 프로그램 설치)로 나뉜다. 기본 커넥터와 **같은 MCP 위에서 동작**하며, 2부에서 이 용어는 그중 기본 커넥터 목록에 준비돼 있지 않은 쪽을 가리키는 좁은 뜻으로 쓴다. | 본 교육 2부 / 출처: `docs/connect/mcp-servers.md` |
