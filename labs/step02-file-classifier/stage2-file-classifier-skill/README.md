...
본 lab의 **학습 초점**은 "Skill에 옵션을 받게 만드는 instructions 설계"이며, 본 교육 [3단계 모델](https://scroogy-dev.github.io/ai-onboarding/#stage-model)의 **stage 2(정형 응답 Skill)** 개념입니다.
...
- 옵션 추가는 **instructions의 "지원 옵션" 블록만 늘리면 끝입니다.** Skill 자체를 새로 만들지 않습니다.
...
| Claude Desktop에서 "파일을 이동했다"고 답한다 | 일반 대화는 파일시스템 접근 권한이 없습니다. 출력 마지막의 "이동 결과"가 `분류만 수행`으로 나오는지 확인하세요. 실제 이동을 원하면 Cowork 또는 Code로 옮겨야 합니다. |
