...
- **날씨**: `맑음/구름조금/구름많음/흐림/비/소나기/천둥번개/눈/안개` 9가지 중 하나
- **바람**: 풍속 수치 (m/s, 예: `2.4`) + 약함/보통/강함 중 하나
- **습도**: 정수 (예: `38`) + 건조/적정/높음 중 하나
...
- **기준 시각**: `<HH>시` 형식의 정수 시 (예: `14`)
- **최고/최저**: 오늘의 최고·최저 정수 두 개 (예: `22 / 11`)
- **날씨 한 줄 설명**: 시즌·기온을 반영한 톤 있는 한 줄 (예: `맑고 따뜻한 봄날`)
- **풍향**: 8방위 (북풍/북동풍/동풍/남동풍/남풍/남서풍/서풍/북서풍 중)
- **강수확률**: 정수 (%)
- **자외선**: 낮음/보통/높음/매우높음 중 하나 + 정수 지수 (예: `높음 / 6`)
- **PM10 농도**: 정수 (㎍/㎥)
- **PM2.5 농도**: 정수 (㎍/㎥)
...
| `{{feels_like}}` | 체감 온도 숫자만, 정수 (예: `21`) | 쾌적·선선 등 한 단어는 `{{desc}}`에 |
| `{{high}}` | 정수 (예: `22`) | |
| `{{low}}` | 정수 (예: `11`) | |
...
- **미세먼지 등급**도 4가지(`좋음`·`보통`·`나쁨`·`매우 나쁨`) 외 다른 표현은 금지한다. `{{pm10}}`·`{{pm25}}`가 그대로 `data-grade` attribute에 박혀 배지 색을 결정한다.
- **우산 표기**는 `필요해요`/`필요없음` 두 단어만 쓴다. `data-need` attribute가 강조 색을 결정한다.
