...
!!! note "저장소를 프로젝트에 통째로 붙이는 별도 경로도 있습니다"
    커넥터와 별개로, 대화창 `+` 버튼의 **GitHub에서 추가**로 저장소를 **프로젝트**(claude.ai에서 자료를 모아 두는 작업 공간) 자료에 붙여 둘 수 있습니다. 이쪽은 **지정한 브랜치의 파일 이름과 내용만** 가져오며 커밋 이력·이슈·PR 같은 기록은 가져오지 않습니다 (이 경로의 공식 안내: [GitHub 통합 사용하기](https://support.claude.com/ko/articles/10167454)).
