...
**목록에 서비스가 안 보인다면 권한 문제일 수 있습니다.** Team·Enterprise 플랜에서는 **조직 관리자가 먼저 커넥터를 켜 주어야** 구성원이 쓸 수 있습니다. 관리자가 조직 설정에서 커넥터를 팀에 추가하고, 서비스별로 **항상 허용 / 승인 필요 / 차단** 같은 도구 권한을 정할 수 있습니다. 목록이 비어 있거나 원하는 서비스가 없다면 내 문제가 아니라 조직 정책일 가능성이 크니, 담당 부서에 문의하세요.
