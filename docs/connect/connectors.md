...
!!! info "학생·일반인: 읽기만이라면 연결하지 않아도 됩니다"
    ...
    커넥터가 필요해지는 것은 **로그인과 권한 확인이 필요한 일**입니다. 로그인해야 보이는 비공개 저장소를 읽거나, 공개 저장소라도 이슈·PR에 코멘트를 남기는 일이 그렇습니다.
...
!!! warning "임직원: 업무 도구가 따로 있다면 연결하지 않는 쪽을 권합니다"
    ...
    커넥터는 **로그인과 권한 확인이 필요한 일**에 쓰는 수단입니다. 회사 자료를 다루는 연결에 공통으로 걸리는 기준(플랜·관리자 정책)은 다른 커넥터와 같습니다 (→ [연결 보안 기준](../security-guide.md#external-connection)).
