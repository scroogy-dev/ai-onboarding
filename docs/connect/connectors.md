...
1부 [기본 용어 ― 하네스는 결국 '내가 세팅하는 것들'](../basics.md#model-harness)에서 하네스의 출처를 셋으로 나눴고, 그중 **「고르고·연결만 하는 것」**에 「외부 서비스 연결」이 있었습니다. 커넥터가 바로 그 실제 예입니다.
...
**쓸 만큼만 켜 두세요.** 같은 메뉴의 **도구 액세스**에서 켜 둔 커넥터를 언제 불러올지 세 가지 중 고를 수 있습니다.
...
켜 둔 커넥터가 많을수록 AI가 엉뚱한 곳을 뒤질 여지도 늘어납니다. 고르는 기준은 [Claude의 도구 접근 관리](https://support.claude.com/ko/articles/13730515)에 정리돼 있습니다.
...
**AI가 쓴 것도 내 이름으로 팀에 공개됩니다.** 내 계정으로 연결했으니 AI가 만든 문서나 코멘트도 **내가 쓴 것으로** 올라갑니다. 초안을 만들게 하되 **올리기 전에 직접 읽고 고치는 단계**를 반드시 두세요. 이것이 [메타 원칙 ③ AI 결과물 검토·이해 의무](../index.md#meta-principles)가 연결 이후에 더 중요해지는 이유입니다.
...
!!! example "GitHub ― 애플리케이션 개발하기"
    - "이 이슈가 요구하는 것을 정리하고 어떤 파일부터 봐야 할지 알려 줘"
    - "이 이슈를 해결하는 코드를 작성해 줘"
    - "이 PR을 코드 리뷰해 줘"

- **읽기만이라면 연결하지 않아도 됩니다.** 공개(Public) 저장소는 계정 없이도 볼 수 있어서, README나 이슈를 읽는 정도는 주소를 알려 주고 물어보면 됩니다. 이 교육의 [실습 자료](../labs.md)도 공개 저장소에 있습니다.
- **업무 도구가 따로 있다면 연결하지 않는 쪽을 권합니다.** 회사에 이슈·업무를 관리하는 도구(Jira·Confluence 등)가 있다면 비개발자가 GitHub까지 연결할 이유는 대개 없습니다. 함께 봐야 하는 내용은 그 도구에서 관리하는 편이 양쪽 모두에게 낫습니다.
- **GitHub Enterprise는 조건이 다릅니다.** 사내에서 운영하는 쪽은 회사 설정에 따라 공개로 열어 둔 저장소도 로그인해야 보일 수 있고, 계정마다 라이선스 비용이 붙습니다. 볼 수 있는지부터 회사 정책이 정합니다.
- **커넥터가 필요해지는 것은 개발 활동에 들어갈 때입니다.** 공개 저장소를 읽는 데서 그치지 않고, 로그인해야 보이는 비공개 저장소를 그때그때 찾아 읽거나 이슈·PR에 내용을 남기는 일이 그렇습니다. 로그인과 권한 확인을 거쳐야 하는 순간부터 커넥터의 몫입니다 (→ [연결 보안 기준](../security-guide.md#external-connection)).
- **연결한다면 범위를 좁혀 두세요.** 저장소·코드 파일 읽기와 이슈·PR 다루기까지 지원합니다. 읽기만 필요하면 승인 화면에서 저장소를 골라 좁히고, 쓰기 작업 승인은 거절하세요. 정확한 범위는 커넥터 목록의 GitHub 항목 설명이 기준입니다.
- **비공개 저장소는 승인이 한 단계 더 있습니다.** GitHub 쪽에서 앱 접근을 승인해야 하고, 조직이 SSO(회사 계정 통합 로그인)를 쓰면 각자 한 번 더 승인해야 합니다.

!!! note "저장소를 프로젝트에 통째로 붙이는 별도 경로도 있습니다"
    커넥터와 별개로, 프로젝트 화면의 **프로젝트 지식** 영역 `+` 버튼에서 **GitHub**를 골라 저장소를 **프로젝트**(claude.ai에서 자료를 모아 두는 작업 공간) 자료에 붙여 둘 수 있습니다. 이 경로는 **지정한 브랜치의 파일 이름과 내용만** 가져오며 커밋 이력·이슈·PR 같은 기록은 가져오지 않습니다 (이 경로의 공식 안내: [GitHub 통합 사용하기](https://support.claude.com/ko/articles/10167454)).
...
**회사 정책을 먼저 확인하고, 회사가 허용한 커넥터만 씁니다.** Team·Enterprise 플랜에서는 **조직 관리자가 먼저 커넥터를 켜 주어야** 구성원이 쓸 수 있습니다. 관리자가 조직 설정에서 커넥터를 팀에 추가하고, 서비스별로 **항상 허용 / 승인 필요 / 차단** 같은 도구 권한을 정할 수 있습니다. 회사는 보안 정책에 따라 커넥터를 막아 두었을 수 있습니다. 허용되지 않은 커넥터는 쓰지 않고, 연결도 회사가 정한 방법으로 합니다. 목록이 비어 있거나 원하는 서비스가 없다면 허용 범위 밖입니다.

**연결해도 되는지는 회사 정책이 정합니다.** 관리자가 켜 둔 목록은 **쓸 수 있는 범위**를 정한 것이고, 그 안에서 **어느 자료를 연결할지는 내가 고릅니다.** 커넥터는 내 계정 권한을 그대로 쓰기 때문에, 이 선택에서 회사 자료가 의도치 않게 밖으로 나갈 수 있습니다.
...
목록에 없는 도구가 필요하다면 다음 페이지로 넘어가세요.
