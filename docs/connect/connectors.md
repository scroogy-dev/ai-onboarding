...
- **GitHub Enterprise는 조건이 다릅니다.** 사내에서 운영하는 쪽은 공개로 열어 둔 저장소라도 로그인해야 보이고, 계정마다 라이선스 비용이 붙습니다. 볼 수 있는지부터 회사 정책이 정합니다.
- **커넥터가 필요해지는 것은 개발 활동에 들어갈 때입니다.** 공개 저장소를 읽는 데서 그치지 않고, 로그인해야 보이는 비공개 저장소를 다루거나 이슈·PR에 내용을 남기는 일이 그렇습니다. 로그인과 권한 확인을 거쳐야 하는 순간부터 커넥터의 몫입니다 (→ [연결 보안 기준](../security-guide.md#external-connection)).
