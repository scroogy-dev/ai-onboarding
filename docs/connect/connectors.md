...
커넥터 화면으로 가는 길은 두 갈래이고, 어느 쪽으로 들어가도 같은 화면이 열립니다.
...
도착하는 곳은 아래 화면입니다. 왼쪽 목록 아래쪽 **사용자 지정 → 커넥터** 자리이고, 이미 연결해 둔 서비스를 점검할 때도 같은 화면을 씁니다.
...
!!! tip "대화 도중이라면 여기서도 열립니다"
    대화창 왼쪽 아래 **`+` 버튼**(또는 `/` 입력)을 누르고 **커넥터**에 마우스를 올리면, 설정으로 나가지 않고 같은 자리에서 바로 열립니다.

    ![대화창 + 버튼의 커넥터 메뉴. 커넥터 추가·관리와 연결된 서비스별 토글, 도구 액세스가 함께 보입니다](../assets/images/claude-desktop-connector-04.png)

    - **connector 추가**: 목록에 없는 것을 직접 등록하는 자리입니다. 여기서는 쓰지 않고 [MCP 서버](mcp-servers.md)에서 다룹니다.
