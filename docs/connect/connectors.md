...
**회사 정책을 먼저 확인하고, 회사가 허용한 커넥터만 씁니다.** Team·Enterprise 플랜에서는 **조직 관리자가 먼저 커넥터를 켜 주어야** 구성원이 쓸 수 있습니다. 관리자가 조직 설정에서 커넥터를 팀에 추가하고, 서비스별로 **항상 허용 / 승인 필요 / 차단** 같은 도구 권한을 정할 수 있습니다. 회사는 보안 정책에 따라 커넥터를 막아 두었을 수 있습니다. 목록이 비어 있거나 원하는 서비스가 없다면 허용 범위 밖이니 우회할 방법을 찾지 않습니다.
