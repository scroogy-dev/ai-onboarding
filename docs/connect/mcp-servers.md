...
!!! tip "먼저 목록부터 둘러보세요"
    주소를 직접 넣기 전에, 필요한 것이 이미 목록에 있는지부터 봅니다. 원격은 **사용자 정의 → 커넥터**의 **`+` → 찾아보기**에서 앞 페이지의 기본 커넥터 목록을, 로컬은 위에서 본 **설정 → 확장 프로그램**에서 확장 프로그램 목록을 볼 수 있습니다. 서로 다른 목록이지만, 어느 쪽이든 주소를 직접 넣는 것보다 안전한 출발점입니다.
