...
내 컴퓨터는 시키기만 할 뿐이고, 실제 접속은 Anthropic 쪽에서 시작합니다. 그래서 조건은 하나입니다: **그 주소가 인터넷에서 열려 있는가.** 사내망 주소는 내가 VPN에 붙어 있어도 닿지 않습니다. 접속을 시작하는 쪽이 내 컴퓨터가 아니기 때문입니다.
...
!!! example "내 컴퓨터에 설정해야 되는 일은 이런 때입니다"
    어떤 때에 계정 등록으로 안 되는지 보면 구분이 빨라집니다. 둘 다 **인터넷 밖에 있는 자료**라는 공통점이 있습니다.
