...
차이는 딱 한 줄입니다. **기본 커넥터는 목록에서 고르고, 이 페이지의 방식은 주소를 직접 넣습니다.** 아래 표와 이 페이지에서 「MCP 서버」라고 할 때는 그중 **주소를 직접 넣어 등록하는 쪽**을 가리킵니다.
...
!!! warning "확장 프로그램은 내 컴퓨터에서 실행되는 프로그램입니다"
    설치한 확장 프로그램은 **일반 프로그램과 같은 권한으로 내 컴퓨터에서 돌아갑니다.** 목록에서 골랐더라도 설치는 새 프로그램을 까는 일과 같은 무게로 판단하세요. 만든 곳이 믿을 만한지 확인하고(기준은 [믿을 만한 서버 고르기](#choosing)와 같습니다), 회사 컴퓨터라면 회사의 프로그램 설치 규정을 그대로 따릅니다.

!!! tip "먼저 목록부터 둘러보세요"
    원격 커넥터는 **사용자 정의 → 커넥터**의 **`+` → 찾아보기**에서, 로컬 확장 프로그램은 위에서 본 **설정 → 확장 프로그램**에서 각각 목록을 볼 수 있습니다. 어느 쪽이든 주소를 직접 넣는 것보다 안전한 출발점입니다.
...
목록에서 고를 때는 검토 표시가 확인의 일부를 대신해 줍니다. 주소를 직접 넣는 자리에는 그 표시가 아예 없으니, **확인이 전부 내 몫**이 됩니다. 등록 전에 다섯 가지만 보세요.
