...
!!! note "내 컴퓨터에 설정 — 내 컴퓨터에서 접속합니다"
    ...
    **접근 범위**: 내 컴퓨터에서 열 수 있는 자료라면 모두입니다. VPN을 켜야 열리는 사내 자료도, 이 컴퓨터에만 저장되는 앱 자료도 여기 들어갑니다.
...
내 컴퓨터는 시키기만 할 뿐이고, 실제 접속은 Anthropic 쪽에서 시작합니다. 그래서 조건은 하나입니다: **그 주소가 인터넷에서 열려 있는가.** 사내망 주소는 내가 VPN에 붙어 있어도 접근하지 못합니다. 접속을 시작하는 쪽이 내 컴퓨터가 아니기 때문입니다.
