...
| 축 | 기본 커넥터 | MCP 서버(사용자 정의 커넥터) |
|---|---|---|
| **어떻게 잇나** | 목록에서 서비스를 고름 | 서버 주소를 입력해 등록 |
...
!!! note "원격 — 인터넷 너머의 서비스"
    클라우드 서비스에 있는 자료를 잇습니다. 웹·데스크톱·모바일 어디서든 같은 연결을 씁니다.

    **이럴 때** — 회사가 쓰는 업무 도구, 협업 서비스처럼 **웹에 있는 자료**

!!! note "로컬 — 내 컴퓨터 안"
    Claude Desktop에 **확장 프로그램**을 설치해 내 컴퓨터의 파일·앱을 잇습니다.
...
!!! note "이용까지가 이 시리즈의 선입니다"
    ...
    관심이 생겼다면 개발자용 공식 문서에서 이어 갈 수 있습니다 — [MCP 안내](https://docs.claude.com/ko/docs/agents-and-tools/mcp).
...
연결은 **한 번에 다 열어 두는 것보다, 지금 하는 일에 필요한 하나부터 잇는 편**이 훨씬 잘 굴러갑니다. 1부에서 골라 둔 반복 작업이 있다면, 그 작업이 어떤 자료를 필요로 하는지부터 짚어 보세요 (→ [2부 개요](index.md)).
