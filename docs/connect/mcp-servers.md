...
!!! info "화면에서는 둘 다 「커넥터」입니다"
    서버 주소를 넣어 등록할 때 쓰는 메뉴 이름은 **사용자 정의 커넥터**입니다. 앞 페이지에서 본 그 화면, 그 목록에서 이어집니다. 다른 기능을 새로 배우는 게 아니라 **같은 문을 다른 방법으로 여는 것**입니다. 내 컴퓨터에 설치하는 방식은 메뉴가 따로 있습니다 (→ [원격과 로컬](#remote-vs-local)).
...
MCP 서버는 **연결을 맡아 주는 작은 프로그램**입니다. 이 프로그램이 **어디서 돌아가느냐**에 따라 등록하는 방법도, 접근할 수 있는 자료의 범위도 달라집니다.

<div class="grid" markdown>

!!! note "원격 — 이미 돌고 있는 서버에 접속"
    서버가 **내 컴퓨터 밖 어딘가에서 이미 돌고 있고**, Claude 쪽에서 그 주소로 접속합니다. 서비스를 만든 회사가 공개한 서버일 수도, 우리 회사가 사내용으로 운영하는 서버일 수도 있습니다.

    **등록**: 주소 한 줄을 넣으면 끝입니다. 내 컴퓨터에 설치할 것이 없어, 한 번 등록하면 웹·데스크톱·모바일 어디서나 같은 연결을 씁니다.

    **접근 범위**: 그 서버가 접근할 수 있는 자료까지입니다. 다만 **서버 주소가 인터넷에서 열려 있어야** Claude가 접속할 수 있고, 접속을 시작하는 쪽이 내 컴퓨터가 아니라서 **내 컴퓨터 안의 파일에는 접근하지 못합니다.**

!!! note "로컬 — 내 컴퓨터에서 도는 서버를 설치"
    **확장 프로그램**을 설치하면 서버가 **내 컴퓨터에서 실행**됩니다. Claude Desktop이 그 프로그램을 직접 돌립니다.

    **등록**: 프로그램을 그 컴퓨터에 설치해야 합니다. 설치한 컴퓨터에서 Claude Desktop을 열었을 때만 동작하고, 다른 기기에서 쓰려면 거기서 다시 설치합니다.

    **접근 범위**: 내 컴퓨터에서 열 수 있는 자료라면 모두 접근할 수 있습니다. 내 폴더의 파일도, 사내망 안에서만 열리는 자료도 여기 들어갑니다.

</div>

!!! note "원격이라고 다 인터넷에 공개된 서비스는 아닙니다"
    원격 서버는 두 갈래로 나옵니다.

    - **서비스가 공개한 서버**: 누구나 쓸 수 있고, 주소는 그 회사 공식 문서·홈페이지에 적혀 있습니다.
    - **회사가 운영하는 사내 서버**: 회사 자료를 다루라고 회사가 마련해 제공하는 것입니다. 주소는 담당 부서가 안내하고, 회사 계정으로 인증해야 열립니다.

    등록 방법은 둘 다 같습니다. 달라지는 것은 **주소를 어디서 받고, 믿을 만한지 누가 판단했는가**입니다. 사내 서버는 회사가 이미 검토해 열어 준 것이니 [고르는 판단](#choosing)을 내가 처음부터 할 필요가 없고, 대신 회사가 정한 절차를 따릅니다.

    단, 사내망 안에서만 열리는 주소는 원격으로 등록해도 연결되지 않습니다. 내가 VPN에 붙어 있어도 마찬가지입니다: **접속을 시작하는 쪽이 내 컴퓨터가 아니기 때문**입니다 (→ [잘 안 될 때](#troubleshooting)).

!!! example "로컬이라야 되는 일은 이런 때입니다"
    원격으로 안 되는 자리가 어디인지 보면 구분이 빨라집니다. 셋 다 **인터넷 밖에 있는 자료**라는 공통점이 있습니다.

    - **내 컴퓨터에만 있는 파일 뭉치**: 폴더에 쌓인 회의록·견적서 수십 개를 대화창에 하나씩 올리는 대신, 폴더를 지정해 한꺼번에 훑어 정리할 때
    - **사내망 안에서만 열리는 자료**: 회사 네트워크나 VPN을 켜야 열리는 사내 위키·파일 서버. 회사가 인터넷 쪽 주소를 따로 열어 두지 않았다면 원격으로는 접근할 수 없습니다
    - **이 컴퓨터에만 저장되는 앱 자료**: 계정 동기화 없이 내 기기에만 남는 메모·일정

    반대로 서버가 내 컴퓨터 밖에서 돌고 있다면 원격으로 충분합니다. 공개된 협업 서비스든 회사가 운영하는 사내 서버든, 내 컴퓨터에 아무것도 설치하지 않고 주소만 등록하면 됩니다.

!!! tip "방식은 내가 고르는 것이 아니라 만든 쪽이 정해서 내놓습니다"
    같은 도구라도 원격 주소로만 나온 것이 있고, 확장 프로그램으로만 나온 것이 있습니다. 그러니 쓰려는 도구가 **어느 방식으로 나와 있는지부터** 봅니다. 둘 다 있다면 내 컴퓨터에 설치하지 않는 원격이 손이 덜 갑니다.

    둘을 같이 써도 됩니다. 내 컴퓨터의 문서를 로컬로 읽고, 정리한 결과를 원격으로 업무 도구에 올리는 식입니다.

    제품마다 지원 범위가 다르니 1부 [도구와 제품 — 기능 비교](../intro.md#claude-comparison) 표도 함께 보세요.

!!! warning "로컬로 연결해도 자료는 Claude 서비스로 보내져 처리됩니다"
    「로컬」은 **서버가 실행되는 위치**를 가리킬 뿐입니다. 서버가 내 컴퓨터에서 돌더라도, Claude가 읽은 내용은 대화에 붙여 넣은 자료와 **똑같이** Claude 서비스로 보내져 처리됩니다. 밖으로 나가면 안 되는 자료라면 로컬 연결이라서 안심할 것이 아니라, 처음부터 연결 대상에서 빼는 것이 맞습니다 (→ [보안 및 개인정보 가이드 — 외부 연결](../security-guide.md#external-connection)).
...
이 문서에서는 Claude Desktop 기준으로 설명합니다. 설정 파일을 만질 필요가 없습니다.
...
!!! tip "열쇠 값(토큰)을 만들 때는 필요한 만큼만"
    ...
    만드는 화면에서 **무엇을 할 수 있는지 골라 주는 서비스가 많습니다.** 고를 수 있다면 지금 필요한 것만 남기세요. 앞 페이지 [승인 화면에서 확인할 것](connectors.md#approval-screen)에서 하던 판단을 발급 자리에서 미리 하는 것이고, 기준도 [연결 보안 — 맡기는 권한은 최소로](../security-guide.md#external-connection)와 같습니다.
...
!!! warning "열쇠 값은 대화창에 붙여넣지 않습니다"
    설정 칸에 넣는 것과 대화에 붙여넣는 것은 다릅니다. **설정 칸은 연결을 세우는 자리**이고, **대화창은 자료가 오가는 자리**입니다. 열쇠 값은 [절대 입력하면 안 되는 정보](../security-guide.md#never-input)의 「인증 정보」에 해당하니, 확장 프로그램을 설정할 때 말고는 어디에도 넣지 마세요.

!!! tip "먼저 목록부터 둘러보세요"
    주소를 직접 넣기 전에, 필요한 것이 이미 목록에 있는지부터 봅니다. 원격은 **사용자 지정 → 커넥터**의 **`+` → 찾아보기**에서 앞 페이지의 기본 커넥터 목록을, 로컬은 위에서 본 **설정 → 확장 프로그램**에서 확장 프로그램 목록을 볼 수 있습니다. 서로 다른 목록이지만, 어느 쪽이든 주소를 직접 넣는 것보다 안전한 출발점입니다.
...
목록에서 고를 때는 검토 표시가 확인의 일부를 대신해 줍니다. 주소를 직접 넣는 자리에는 검토를 마쳤다는 표시가 붙지 않으니, **확인이 전부 내 몫**이 됩니다. 등록 전에 다섯 가지만 보세요.
...
!!! abstract "판단이 늘어난 만큼 검토도 늘어납니다"
    MCP 서버는 커넥터보다 폭이 넓고, 넓어진 만큼 AI가 예상 밖의 자료에 접근할 여지도 커집니다. [메타 원칙 ③ AI 결과물 검토·이해 의무](../index.md#meta-principles)가 가장 무거워지는 자리가 여기입니다.
...
| 주소를 넣었는데 연결 실패 | 사내망·VPN 안에서만 열리는 주소는 아닌지. 연결은 내 컴퓨터가 아니라 **Claude 쪽 서버에서 시작**하므로, 밖에서 접속할 수 없는 주소는 내 컴퓨터에서 열리더라도 연결되지 않습니다 |
