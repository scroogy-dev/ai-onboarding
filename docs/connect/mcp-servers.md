...
!!! note "내 컴퓨터에 설정 — 내 컴퓨터에서 접속합니다"
    ...
    **닿는 범위**: 내 컴퓨터가 닿는 곳이면 모두입니다. VPN을 켜야 열리는 사내 자료도, 이 컴퓨터에만 저장되는 앱 자료도 여기 들어갑니다.
...
!!! info "내 컴퓨터에 설정한다고 서버까지 내 컴퓨터에 있는 것은 아닙니다"
    ...
    - **설정도 내 컴퓨터, 서버도 내 컴퓨터**: 기기의 메모 앱을 읽는 도구처럼 프로그램이 통째로 내 기기에서 실행되는 경우
...
!!! example "내 컴퓨터에 설정해야 되는 일은 이런 때입니다"
    계정 등록으로 안 되는 자리가 어디인지 보면 구분이 빨라집니다. 둘 다 **인터넷 밖에 있는 자료**라는 공통점이 있습니다.
