...
!!! info "화면에서는 둘 다 「커넥터」입니다"
    서버 주소를 넣어 등록하는 자리는 앞 페이지에서 본 **사용자 지정 → 커넥터** 화면 그대로입니다. 거기서 목록에 있는 서비스를 고르면 기본 커넥터, **`+`** → **사용자 정의 커넥터 추가**로 주소를 넣으면 이 페이지의 방식입니다. 새 기능이 아니라 **같은 화면에서 등록하는 방법만 다른 것**입니다. 내 컴퓨터에 설치하는 방식은 메뉴가 따로 있습니다 (→ [원격과 로컬](#remote-vs-local)).
