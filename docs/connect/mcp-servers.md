...
등록 방식은 **연결 통로가 어디서 실행되느냐**에 따라 갈립니다. 이름은 낯설어도 구분은 간단합니다.

<div class="grid" markdown>

!!! note "원격 — 남이 띄워 둔 통로에 접속"
    통로가 인터넷 어딘가에서 이미 돌고 있고, **Claude 쪽 서버가 그 주소로 접속**합니다. 내 컴퓨터에는 아무것도 설치하지 않아 웹·데스크톱·모바일 어디서든 같은 연결을 씁니다.

    **주로 이럴 때**: 회사가 쓰는 업무 도구, 협업 서비스처럼 인터넷으로 열려 있는 서비스

!!! note "로컬 — 내 컴퓨터에 깔아서 실행"
    **확장 프로그램**을 설치하면 통로가 **내 컴퓨터에서 실행**됩니다. Claude Desktop이 그 프로그램을 직접 돌립니다.

    **주로 이럴 때**: 내 컴퓨터의 파일·앱을 다뤄야 하거나, 사내망 안에서만 열리는 자료가 필요할 때
...
!!! warning "로컬로 연결해도 자료는 Claude 서비스로 보내져 처리됩니다"
    「로컬」은 **통로가 실행되는 위치**를 가리킬 뿐입니다. 통로가 내 컴퓨터에서 돌더라도, Claude가 읽은 내용은 대화에 붙여 넣은 자료와 **똑같이** Claude 서비스로 보내져 처리됩니다. 밖으로 나가면 안 되는 자료라면 로컬 연결이라서 안심할 것이 아니라, 처음부터 연결 대상에서 빼는 것이 맞습니다 (→ [보안 및 개인정보 가이드 — 외부 연결](../security-guide.md#external-connection)).
...
!!! info "회사 플랜이라면 관리자가 먼저 열어 줘야 합니다"
    Team·Enterprise 플랜에서는 **조직 소유자만** 사용자 정의 커넥터를 조직에 추가할 수 있습니다. 소유자가 **조직 설정 → 커넥터**에서 추가하고 나면, 구성원은 **사용자 지정 → 커넥터** 목록에서 그 커넥터를 찾아 **연결**만 누르면 됩니다.
...
!!! warning "확장 프로그램은 내 컴퓨터에서 실행되는 프로그램입니다"
    설치한 확장 프로그램은 **일반 프로그램과 같은 권한으로 내 컴퓨터에서 실행됩니다.** 목록에서 골랐더라도 설치는 새 프로그램을 까는 일과 같은 무게로 판단하세요. 만든 곳이 믿을 만한지 확인하고(기준은 [믿을 만한 서버 고르기](#choosing)와 같습니다), 회사 컴퓨터라면 회사의 프로그램 설치 규정을 그대로 따릅니다.
