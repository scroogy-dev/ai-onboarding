...
방식은 흔히 **원격**과 **로컬**로 부릅니다. 이름만 보면 서버가 어디에 위치하느냐의 문제 같지만, 실제로 판가름하는 것은 **설정을 어디에 두느냐**입니다. 거기서 **누가 접속하는지**가 정해지고, 그것이 무엇에 닿을 수 있는지를 결정합니다.
...
!!! info "내 컴퓨터에 설정한다고 서버까지 내 컴퓨터에 있는 것은 아닙니다"
    가장 헷갈리는 지점입니다. **설정을 두는 자리**와 **서버가 실행되는 자리**는 별개입니다.

    - **설정도 내 컴퓨터, 서버도 내 컴퓨터**: 파일 정리 도구처럼 프로그램이 통째로 내 기기에서 실행되는 경우
