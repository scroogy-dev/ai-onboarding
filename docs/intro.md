...
> 표의 **✓ 원격**은 내 컴퓨터 밖에서 돌고 있는 서버에 접속하는 방식으로, 내 컴퓨터에 설치할 것이 없어 웹·데스크톱·모바일 어디서나 같습니다. **△ 로컬**은 연결을 맡는 프로그램을 내 컴퓨터에서 직접 돌리는 방식이라 **Claude Desktop 앱에서만** 동작합니다. 대화든 Cowork든 데스크톱 앱에서 쓸 때만 되고 웹·모바일에서는 쓸 수 없지만, 대신 내 컴퓨터 안의 파일처럼 인터넷 밖에 있는 자료에도 접근할 수 있습니다. 두 방식의 차이와 고르는 기준은 **[2부 — 원격과 로컬](connect/mcp-servers.md#remote-vs-local)** 에서 다룹니다.
