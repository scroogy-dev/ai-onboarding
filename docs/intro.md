...
생성형 AI 제품은 **사용 방식**으로도 두 갈래로 나눠 볼 수 있습니다. 이 절이 **챗봇·에이전트가 각각 무엇인지 정하는 자리**이며, 다른 페이지에서 두 단어가 나오면 모두 여기 정의를 가리킵니다. 본 교육의 [3단계 모델](index.md#stage-model)과도 직접 연결됩니다.
...
!!! quote "왜 이 분류가 중요한가요?"
    챗봇(물어보기)과 에이전트(맡기기)는 **반복 작업에서의 비용 구조**가 다릅니다. 같은 일을 매번 새로 시킬지, 한 번 맡길 수 있게 만들어 둘지가 갈리죠.
