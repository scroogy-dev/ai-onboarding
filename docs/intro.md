...
> 표의 **✓ 원격**은 **내 계정에 등록**해 쓰는 방식입니다. 접속을 Anthropic 쪽에서 하므로 내 컴퓨터에 설치할 것이 없고 웹·데스크톱·모바일 어디서나 같지만, 인터넷에서 열려 있는 주소에만 닿습니다. **△ 로컬**은 **내 컴퓨터에 설정**해 쓰는 방식이라 **Claude Desktop 앱에서만** 동작합니다. 대신 접속이 내 컴퓨터에서 나가므로, 내 컴퓨터 안의 파일이나 사내망 안에서만 열리는 자료에도 접근할 수 있습니다. 두 방식의 차이와 고르는 기준은 **[2부 — 어디에 설정하느냐](connect/mcp-servers.md#remote-vs-local)** 에서 다룹니다.
...
작업 세션은 클라우드에서 돌아가 웹·데스크톱·모바일 어디서나 이어서 볼 수 있습니다. 다만 **내 컴퓨터의 파일을 직접 다루는 일과 로컬 연결은 데스크톱 앱에서만** 됩니다 (→ [기능 비교](#claude-comparison)).
