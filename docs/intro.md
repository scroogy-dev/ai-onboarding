...
> 실제로 연결하는 방법은 **[2부 — 도구·데이터 연결](connect/index.md)** 에서 다룹니다. 인터넷 너머의 서버에 접속하는 **원격 연결**은 웹·데스크톱 어디서나 같지만, **내 컴퓨터에서 실행**되는 로컬 확장 프로그램은 **Claude Desktop 앱**에서 설치합니다 (→ [2부 — 원격과 로컬](connect/mcp-servers.md#remote-vs-local)).
