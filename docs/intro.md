...
> 표의 **✓ 원격 연결**은 내 컴퓨터 밖에서 돌고 있는 서버에 접속하는 방식으로, 내 컴퓨터에 설치할 것이 없어 웹에서 쓰든 데스크톱에서 쓰든 같습니다. 연결을 맡는 프로그램을 **내 컴퓨터에서 직접 돌리는 방식**도 있는데, 그 프로그램은 Claude Desktop 앱 안에 따로 설치하며, 내 컴퓨터 안의 파일처럼 인터넷 밖에 있는 자료에도 접근할 수 있습니다. 두 방식의 차이와 고르는 기준은 **[2부 — 원격과 로컬](connect/mcp-servers.md#remote-vs-local)** 에서 다룹니다.
