...
> 세 제품 모두 **유료 플랜**이 필요합니다 — 개인은 **Claude Pro 이상**, 임직원은 **회사 Team·Enterprise**(개인 플랜 사용 금지). 본 교육의 Cowork·Code 실습에는 **Claude Desktop 설치**가 전제됩니다. [홈 — 준비사항](index.md#preparation) 참조.
...
> 표의 **✓ 원격 · △ 로컬**은 **연결할 MCP 서버가 어디에서 실행되느냐**의 구분입니다. **원격**은 서버가 **내 컴퓨터 밖**에서 실행되는 경우 전부로, 기본 커넥터로 연결하는 서비스도, 인터넷에 공개된 MCP 서버도, 회사가 **사내망**에 제공하는 MCP 서버도 모두 여기에 속합니다. **로컬**은 서버가 **내 컴퓨터 안**에서 실행되는 경우로, 쓰려는 컴퓨터마다 설치해야 하니 **Claude Desktop 앱에서만** 동작해 △입니다. 서버 위치와 별개로 **설정을 어디에 두느냐**의 구분이 따로 있습니다: 내 계정에 등록하는 방식은 접속을 Anthropic 쪽에서 하므로 웹·데스크톱·모바일 어디서나 같지만 인터넷에서 열려 있는 주소만 연결되고, 사내망 서버는 로컬 서버처럼 **내 컴퓨터에 설정**해 내 컴퓨터에서 접속합니다 (그래서 이때도 데스크톱 앱이 필요합니다). 두 구분이 어떻게 맞물리는지는 **[2부 — 어디에 설정하느냐](connect/mcp-servers.md#remote-vs-local)** 에서 다룹니다.
...
작업 세션은 클라우드에서 실행되어 웹·데스크톱·모바일 어디서나 이어서 볼 수 있습니다. 다만 **내 컴퓨터의 파일을 직접 다루는 일과 내 컴퓨터에 설정하는 연결은 데스크톱 앱에서만** 됩니다 (→ [기능 비교](#claude-comparison)).

!!! tip "본 교육에서의 위치"
    **3단계 본격 실습 도구** — 비개발자 친화적 인터페이스로, 본 교육의 3단계 실습은 Cowork으로 진행합니다. 실습에서 내 컴퓨터의 파일을 다루므로 [Claude Desktop 설치](index.md#preparation)가 필요합니다.
