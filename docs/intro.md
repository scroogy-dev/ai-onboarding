...
> 표의 **✓ 원격 · △ 로컬**은 **연결할 MCP 서버가 어디서 도느냐**의 구분입니다. **원격**은 서버가 **내 컴퓨터 밖**에서 도는 경우 전부로, 기본 커넥터로 연결하는 서비스도, 인터넷에 공개된 MCP 서버도, 회사가 **사내망**에 제공하는 MCP 서버도 모두 여기에 속합니다. **로컬**은 서버가 **내 컴퓨터 안**에서 도는 경우로, 쓰려는 컴퓨터마다 설치해야 하니 **Claude Desktop 앱에서만** 동작해 △입니다. 서버 위치와 별개로 **설정을 어디에 두느냐**의 구분이 따로 있습니다: 내 계정에 등록하는 방식은 접속을 Anthropic 쪽에서 하므로 웹·데스크톱·모바일 어디서나 같지만 인터넷에서 열려 있는 주소만 연결되고, 사내망 서버는 로컬 서버처럼 **내 컴퓨터에 설정**해 내 컴퓨터에서 접속합니다 (그래서 이때도 데스크톱 앱이 필요합니다). 두 구분이 어떻게 맞물리는지는 **[2부 — 어디에 설정하느냐](connect/mcp-servers.md#remote-vs-local)** 에서 다룹니다.
...
작업 세션은 클라우드에서 돌아가 웹·데스크톱·모바일 어디서나 이어서 볼 수 있습니다. 다만 **내 컴퓨터의 파일을 직접 다루는 일과 내 컴퓨터에 설정하는 연결은 데스크톱 앱에서만** 됩니다 (→ [기능 비교](#claude-comparison)).
