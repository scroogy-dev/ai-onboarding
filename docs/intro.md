...
!!! info "Cowork의 3중 보호 모델"
    Anthropic 공식 안내에 따르면 Cowork는 **계층화된 보호**로 실행됩니다.

    - **코드 실행 격리** — Claude가 작성한 셸 명령·코드는 사용자 OS와 분리된 **격리된 가상 머신(VM)** 안에서만 실행됩니다.
    - **제어된 파일 액세스** — 사용자가 명시적으로 **연결한 폴더의 파일만** 읽기·쓰기 가능합니다.
    - **제어된 네트워크 액세스** — 외부 호출은 사용자·관리자가 **구성한 송신(egress) 설정**에 따릅니다 (기본은 보수적이며, 필요한 도메인을 명시적으로 허용하는 식).
